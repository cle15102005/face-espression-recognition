import numpy as np
import data_loader, eval, svc, rf, src
import warnings
...
def detect(model_name):
    ...
    if option == "Y":
        idx = np.random.choice(len(X_val_raw), size= 600, replace=False)
    ...
    #transform validation dataset for SRC
    if model_name == 'SRC':
        print("(+) Transforming validation set...")
        X_val = detector.transform(X_val_raw)
    else:
        X_val = X_val_raw
...
if __name__ == '__main__':
    model_name = input('Enter model name (SVC/RF/SRC): ').upper()
    detect(model_name)